class CustomAppModule extends HTMLElement {
    ...
    async loadResources(module) {
        const promises = [];
        if (this.parameters.templateUrl) promises.push(fetch(`src/modules/${module}/${module}.view.html`)
        ...
        this.parameters.stylesUrl.map((styleUrl) => {
            promises.push(fetch(`src/modules/${module}/${styleUrl.replace('scss', 'css')}`)
                .then(response => response.text())
                .then(css => {
                    styles.remove();
