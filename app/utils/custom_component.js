class CustomAppComponent extends HTMLElement {
    ...
    async loadResources(component) {
        const promises = [];
        if (this.parameters.templateUrl) promises.push(fetch(`/app/components/${component}/${component}.view.html`)
        ...
        this.parameters.stylesUrl.map((styleUrl) => {
            promises.push(fetch(`/app/components/${component}/${styleUrl.replace('scss', 'css')}`)
                .then(response => response.text())
                .then(css => {
                    styles.remove();
